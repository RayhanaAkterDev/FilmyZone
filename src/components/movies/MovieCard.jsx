    ...
    return (
        <article className="group min-w-0 cursor-pointer">
            ...
            <div className="relative aspect-2/3 overflow-hidden rounded-2xl bg-input ring-1 ring-black/5">
                ...
                {/* Rating */}
                <div className="absolute right-3 top-3 flex items-center gap-1 rounded-lg bg-black/45 px-2 py-1.5 text-[11px] font-semibold text-white backdrop-blur-md">
                    <Star size={11} fill="currentColor" className="text-rating" />

                    <span>{movie.rating}</span>
                </div>

                {/* Hover content */}
                <div className="absolute inset-0 flex flex-col justify-end bg-linear-to-t from-black/80 via-black/10 to-transparent p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                    <div className="translate-y-3 transition-transform duration-300 group-hover:translate-y-0">
                        <div className="flex items-center gap-2">
                            <button
                                type="button"
                                onClick={(event) => event.stopPropagation()}
                                className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-white shadow-lg transition-transform hover:scale-105 hover:bg-primary-hover"
                            >
                                <Play size={15} fill="currentColor" className="ml-0.5" />
                            </button>

                            <span className="text-xs font-medium text-white/85">View details</span>
                        </div>
                    </div>
                </div>
            </div>
